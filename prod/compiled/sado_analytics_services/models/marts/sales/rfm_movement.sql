...
source AS (
  ...
  FROM
    `agile-scheme-394814`.`dbt_dev_sales`.`revenue`
...
FROM
  ...
  last_branch as (
    ...
    from `agile-scheme-394814`.`dbt_dev_sales`.`revenue`
    where customer_id is not null
  ...
  case when date_trunc(first_purchase,month) = start_of_month then "Khách mới" else "Khách cũ" end as recency_type,
