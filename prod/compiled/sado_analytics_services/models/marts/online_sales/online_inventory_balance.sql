...
calendar AS (
  SELECT
    ...
    C.date,
  FROM
    `agile-scheme-394814`.`dbt_dev`.`calendar` C
    CROSS JOIN `agile-scheme-394814`.`dbt_dev`.`stg_nhanhvn__products`
    p
    CROSS JOIN `agile-scheme-394814`.`dbt_dev`.`stg_nhanhvn__depots`
    d
  WHERE
    C.date <= CURRENT_DATE()
...
daily_sales AS (
  ...
  FROM
    `agile-scheme-394814`.`dbt_dev_online_sales`.`orders_items`
    group by 1,2
),
daily_bills as (
  select 
  ...
  from `agile-scheme-394814`.`dbt_dev`.`stg_nhanhvn__bills`
...
FROM
  ...
  AND inv.date = s.date
  left join daily_bills b on inv.product_id = b.product_id and inv.date = b.bill_date