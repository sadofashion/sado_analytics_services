...
FROM
    `agile-scheme-394814`.`dbt_dev`.`stg_facebookads__adsinsights` adsinsights
    left join `agile-scheme-394814`.`dbt_dev`.`stg_facebookads__campaigns` campaigns on adsinsights.campaign_id = campaigns.campaign_id
